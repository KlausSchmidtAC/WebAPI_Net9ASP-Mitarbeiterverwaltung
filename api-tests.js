    ...
    scenarios: {
        api_test: {
            ...
            stages: [
                ...
                { duration: '30s', target: 50  },  // nachhaltiger Betrieb
                { duration: '30s', target: 75  },  // Peak (≈ Max-Kapazität)
                { duration: '30s', target: 150 },  // Überlast (optional, um Stabilität zu testen)
                { duration: '30s', target: 250 },  // Hohe Überlast
                { duration: '30s', target: 500 },  // Halten der hohen Überlast
                { duration: '30s', target: 50  },  // abkühlen
...
export default function () {
    ...
    // Timeout-Params: nach 10s aufgeben statt 60s hängen → verhindert client-seitige Timeouts
    const p     = { timeout: '10s' };
